...
def main():
    ...
    klant = "Stiek"
    script = "E-Uur | Looncomponenten | Volledige Update"
    bron = 'Python'
            ...
            if klantnaam == "Stiek":            
                ...
                for id, werknemer, actief in zip(plaatsing_df['ID'], plaatsing_df['Werknemer'], plaatsing_df['Actief']):
                    if actief == True:
                        ...
                        # Data overdracht
                        empty_and_fill_table(converted_df, "Looncomponenten", id, klant_connection_string, greit_connection_string, klant, bron, script, script_id)

                    else:
                        print(f"Geen looncomponenten ophalen voor {id} {werknemer}: is niet actief")
                        log(greit_connection_string, klant, bron, f"Geen looncomponenten ophalen voor {id} {werknemer}: is niet actief", script, script_id)
