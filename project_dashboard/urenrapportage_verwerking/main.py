...
def main():
    ...
    # Start logging
    bron = 'python'
    logging(greit_connection_string, klant, bron, f"Script gestart", script, script_id)
    ...
    if database_conn:
        ...
        if connection_dict:
            ...
            # Start logging
            logging(greit_connection_string, klant, bron, f"Ophalen connectiestrings gestart", script, script_id)
        else:
            # Foutmelding logging
            print(f"FOUTMELDING | Ophalen connectiestrings mislukt na meerdere pogingen")
            logging(greit_connection_string, klant, bron, f"FOUTMELDING | Ophalen connectiestrings mislukt na meerdere pogingen", script, script_id)
    else:
        # Foutmelding logging
        print(f"FOUTMELDING | Verbinding met database mislukt na meerdere pogingen")
        logging(greit_connection_string, klant, bron, f"FOUTMELDING | Verbinding met database mislukt na meerdere pogingen", script, script_id)
    ...
    for klantnaam, (klant_connection_string, type) in connection_dict.items():
        ...
        if type != 1:
            ...
            logging(greit_connection_string, klant, bron, f"Skip {klantnaam}", script, script_id)
            continue
        ...
        if klantnaam == "Stiek":      
            ...
            try:
                ...
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Excel bestand ophalen mislukt: {e}", script, script_id)
                return
            ...
            try:
                ...
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Excel bestand opschonen mislukt: {e}", script, script_id)
                return
            ...
            try:
                ...
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Excel verwerking mislukt: {e}", script, script_id)

            # Dataframe check
            if df is None:
                print(f"FOUTMELDING | Geen DataFrame geretourneerd")
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Geen DataFrame geretourneerd", script, script_id)
                return
            if df.empty:
                print(f"FOUTMELDING | DataFrame is leeg")
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | DataFrame is leeg", script, script_id)
                return
            ...
            for tabel, typing in column_typing.items():
                    ...
                    try:
                        ...
                        logging(greit_connection_string, klant, bron, f"Kolommen type conversie correct uitgevoerd", script, script_id, tabel)
                    except Exception as e:
                        print(f"FOUTMELDING | Kolommen type conversie mislukt: {e}")
                        logging(greit_connection_string, klant, bron, f"FOUTMELDING | Kolommen type conversie mislukt: {e}", script, script_id, tabel)
                        return
            ...
            for mapping_table, mapping in column_mapping.items():
                    ...
                    try:
                        ...
                        logging(greit_connection_string, klant, bron, f"Mapping van kolommen correct uitgevoerd", script, script_id, tabel)
                    except Exception as e:
                        print(f"FOUTMELDING | Kolommen transformeren mislukt: {e}")
                        logging(greit_connection_string, klant, bron, f"FOUTMELDING | Kolommen transformeren mislukt: {e}", script, script_id, tabel)
                        return
            ...
            try:
                ...
                logging(greit_connection_string, klant, bron, f"Tabel {tabel} leeg gemaakt van {begindatum} tot {einddatum}", script, script_id, tabel)
            except Exception as e:
                print(f"FOUTMELDING | Tabel leeg maken mislukt: {e}")
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Tabel leeg maken mislukt: {e}", script, script_id, tabel, detail)
                return
            
            # Tabel vullen
            try:
                print(f"Volledige lengte {tabel}: ", len(df))
                logging(greit_connection_string, klant, bron, f"Volledige lengte {tabel}: {len(df)}", script, script_id, tabel)
                added_rows_count = write_to_database(df, tabel, klant_connection_string)
                print(f"Tabel {tabel} gevuld met {detail}")
                logging(greit_connection_string, klant, bron, f"Tabel gevuld met {added_rows_count} rijen met {detail}", script, script_id,  tabel)
            except Exception as e:
                print(f"FOUTMELDING | Tabel vullen mislukt: {e}")
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Tabel vullen mislukt: {e}", script, script_id,  tabel)
                return
            ...
            try:
                ...
                logging(greit_connection_string, klant, bron, f"Bestand succesvol verwijderd: {filepath}", script, script_id,  tabel)
            except Exception as e:
                print(f"FOUTMELDING | Bestand verwijderen mislukt: {e}")
                logging(greit_connection_string, klant, bron, f"FOUTMELDING | Bestand verwijderen mislukt: {e}", script, script_id,  tabel)
                return
    ...
    logging(greit_connection_string, klant, bron, f"Script gestopt in {tijdsduur_str}", script, script_id)
