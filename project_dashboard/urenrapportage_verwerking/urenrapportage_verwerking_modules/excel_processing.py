...
def get_file_path():
    directory = '/Users/maxrood/werk/greit/klanten/stiek/project_dashboard/urenrapportage_verwerking/urenrapportage_verwerking_data_file'
    ...
    for bestand in os.listdir(directory):
        # Controleer of het bestand voldoet aan het patroon "Urenrapportage_begindatum_einddatum.xlsx"
        if re.match(r"Urenrapportage_\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2}\.xlsx", bestand):
            ...
            match = re.search(r"Urenrapportage_(\d{4}-\d{2}-\d{2})_(\d{4}-\d{2}-\d{2})\.xlsx", bestand)
            if match:
    ...
    if filepath is None:
        raise FileNotFoundError("Geen bestand gevonden dat voldoet aan het patroon 'Urenrapportage_begindatum_einddatum.xlsx' in de directory.")
