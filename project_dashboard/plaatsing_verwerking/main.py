...
import time
from plaatsing_verwerking_modules.log import log
from plaatsing_verwerking_modules.database import connect_to_database, write_to_database, clear_table
from plaatsing_verwerking_modules.config import fetch_script_id, fetch_all_connection_strings, fetch_configurations
from plaatsing_verwerking_modules.excel_processing import process_excel_file, clean_excel, delete_excel_file
from plaatsing_verwerking_modules.table_mapping import transform_columns, plaatsing
from plaatsing_verwerking_modules.type_mapping import convert_column_types, plaatsing_typing
import logging


def main():
    if os.path.exists("/Users/maxrood/werk/greit/klanten/stiek/project_dashboard/plaatsing_verwerking/.env"):
        load_dotenv()
    ...
    for klantnaam, (klant_connection_string, type) in connection_dict.items():
        ...
        if klantnaam == "Stiek":            
            ...
            bron = 'python'
            log(greit_connection_string, klant, bron, f"Script gestart", script, script_id)
            print(f"Script gestart")

            # Excel bestand ophalen
            bron = 'excel'
            filepath = '/Users/maxrood/werk/greit/klanten/stiek/project_dashboard/plaatsing_verwerking/plaatsing_verwerking_data_file/Plaatsing.xlsx'
            ...
            try:
                rows_deleted = clear_table(klant_connection_string, tabel, id_list)
